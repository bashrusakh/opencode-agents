...
Do not re-run expensive final review after every intermediate batch. For the final candidate: establish Candidate HEAD, refresh/record Base SHA, run final local validation, then require `@reviewer` to review the **entire Base-SHA-to-Candidate-HEAD change before push**. Refresh base before publication and again before Ready; if it moved, apply the root base-drift rule before reusing evidence. Push only the exact reviewed head while Draft, verify remote head identity, run remote CI/status, refresh provenance/metadata, then move the owned PR to Ready.
...
The final report must include the canonical PR URL; for owned-PR readiness also include Reviewed Base SHA, Candidate HEAD, current remote HEAD, and identity status.
