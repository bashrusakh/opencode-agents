...
Use @explore when the code path is not identified. Use @tester when reproduction or verification is needed. Use @debugger for root cause and the smallest right-level fix. When the normalized behavior spans multiple meaningful states/paths/consumers/boundaries, map the relevant cases to verification before implementation; skip this for a truly local single-path fix. Use @reviewer for diffs touching shared behavior, security, data handling, API contracts, concurrency, or logic with edge cases/state transitions not obvious from one local function.
